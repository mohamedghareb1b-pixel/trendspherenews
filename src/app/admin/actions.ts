...
/** بيحول جدول حفلات "ارتيكل 2" (JSON جوه hidden input) لمصفوفة، وبيشيل أي صف بدون تاريخ */
function parseTourEventsField(raw: FormDataEntryValue | null): TourEvent[] {
  if (!raw) return [];
  try {
    const parsed = JSON.parse(String(raw));
...
export async function createTourArticleAction(formData: FormData) {
  await requireAdminSession();
  ...
  const article = await container.createArticle.execute({
    ...
    faq: parseFaqField(formData.get("faq")),
    type: "tour",
    tourEvents: parseTourEventsField(formData.get("tourEvents")),
  });
  ...
  revalidatePath("/admin/tour-articles");
  redirect(`/admin/tour-articles/${article.id}`);
}

export async function updateTourArticleAction(articleId: string, formData: FormData) {
  await requireAdminSession();
  ...
  revalidatePath(`/admin/tour-articles/${articleId}`);
  revalidatePath("/admin/tour-articles");
